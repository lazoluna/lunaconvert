...
require ffmpeg mktorrent
  ...
  #defaults
  INPUT_FOLDER=/mnt/sda1/.../lunaconvert/input #specify input folder
  OUTPUT_FOLDER=/mnt/sda1/.../lunaconvert/output #specify output folder
  ...
  ANNOUNCE_URL[3]=http://tracker.example.dp/boyonboy.php/b9ee52eade0d769dd8676b32c696d30a9117010fde23f8b6/announce
  ANNOUNCE_FLAG[3]=GB
  ANNOUNCE_URL[8]=http://random.a2m/eade0d769dd8676b32c696d30a9117010f/announce
...
pcsz ()
{
	clear
	echo " "
	echo " Chose the piece size for making torrents."
	echo "    1024 KiB or lower is fine in most cases."
	echo "    only change if you know what you are doing"
	echo " "
	echo " (1) =   512 KiB for filesizes between 512 MiB - 1024 MiB"
	echo " (2) =  1024 KiB for filesizes between 1 GiB - 2 GiB"
	echo " (3) =  2048 KiB for filesizes between 2 GiB - 4 GiB"
	echo " (4) =  4096 KiB for filesizes between 4 GiB - 8 GiB"
	echo " (5) =  8192 KiB for filesizes between 8 GiB - 16 GiB"
	echo " (6) = 16384 KiB for filesizes between 16 GiB - 512 GiB"
	echo " "
	echo " (0) = leave it at "$b""
	echo " "
	read -r -p "choice: " j
	case $j in
		[1])
			echo " "
			psz=19
			b=" piece size of 512 KiB"
			clear
			;;
		[2])
			echo " "
			psz=20
			b=" piece size of 1024 KiB"
			clear
			;;
		[3])
			echo " "
			psz=21
			b=" piece size of 2048 KiB"
			clear
			;;
		[4])
			echo " "
			psz=22
			b=" piece size of 4096 KiB"
			clear
			;;
		[5])
			echo " "
			psz=23
			b=" piece size of 8192 KiB"
			clear
			;;
		[6])
			echo " "
			psz=24
			b=" piece size of 16384 KiB"
			clear
			;;
		[0])
			echo " "
...
anotherurl ()
{
	echo " (1) make torrents with another announce url / piece size"
	echo " (0) end"
	echo " "
	read -r -p "choice: " j
	case $j in
		[1])
			...
			for in in *; do
				if [ -d "$in" ]; then
					...
					mktorrent -p -s "$FLAG" -l "$psz" -a $ANNOUNCE -o "$torrent" $in
				fi
			done
				if [ "$in" = "*" ]; then
					echo " "
...
FLAG="${arrsortf[1]}"
psz=19
b=" default piece size"
#stayinwonderland? No, take "one"
redpill=0
fastmenu=0
exit=0
flac_24=0
...
while true; do
  ...
  echo " "
  echo " "
  echo " (6) change announce url / piece size"
  ...
  echo "    -> using "$b""
  if [ "$redpill" == "0" ]; then
	echo "    -> you need to leave wonderland, functionality is limited"
  fi
  echo " "
  echo " (x) clear folder"
  echo " "
		...
		cd "$OUTPUT_FOLDER"
			for name in "$FLAC2496_NAME"
				do
					...
					[ -f "$torrent" ] && rm "$torrent"
					mktorrent -p -s "$FLAG" -l "$psz" -a $ANNOUNCE -o "$torrent" $name
				done
			echo " "
		fi
		if [ "$flac_samp" = "1" ]; then
...
{
	echo " "
	echo " (1) encode one specific folder in Input"
	echo " (2) encode one specific folder in Downloads"
...
echo "    -> using "$b""
echo " "
echo ""This will overwrite torrents if already exist""
	echo " "
	echo " (1) proceed"
	echo " (2) change announce / piece size"
	echo " (0) exit to main menu"
	echo " "
	read -r -p "choice: " j
case $j in
...
for in in *; do
	if [ -d "$in" ]; then
		...
		set -euo pipefail
		IFS=$'\n'
		torrent="$OUTPUT_FOLDER/youstayinwonderland.torrent"
		[ -f "$torrent" ] && rm "$torrent"
		mktorrent -p -s wonderland -l "$psz" -a wonderland -o "$torrent" $in
	fi
done
...
echo " "
	if [ "$in" = "*" ]; then
		...
		echo "  #####>>>>> \""$OUTPUT_FOLDER"\" is empty"
		echo " "
		read -n 1 -s -r -p "press any key for main menu"
	else
	echo " "
	echo "finished making torrents using:"
	echo "    -> "${ANNOUNCE:0:40}...""
	if [ "$FLAG" != "" ]; then
		echo "    -> flag: -s "$FLAG""
		else
...
#####################################################################################################
	...
	echo " (3) clear input + output folder"
	echo " (0) back to menu"
	echo " "
	read -r -p "choice: " j
	case $j in
		...
		[3])
			...
			echo " "
			echo "   #####>>>>>  finished succesfully"
			echo "   #####>>>>>  input and output folder are now empty"
			read -n 1 -s -r -p  "   #####>>>>>  hit any key for menu"
			clear;;
		[0])
			clear;;
		*)
